...
class SmallHomeWidgetProvider : HomeWidgetProvider() {
    override fun onUpdate(
        ...
        appWidgetIds.forEach { widgetId ->
            val views = RemoteViews(context.packageName, R.layout.small_widget_layout).apply {
                // 색상 프리셋 배경
                ...
                setInt(R.id.small_widget_container, "setBackgroundResource", backgroundResId)

                // 제목
                val titleText = widgetData.getString("title_text", "목표 설정") ?: "목표 설정"
                setTextViewText(R.id.small_title_text, titleText)

                // D-Day
                val ddayText = widgetData.getString("dday_text", "D-0") ?: "D-0"
                setTextViewText(R.id.small_dday_text, ddayText)

                // 퍼센트
                val percentText = widgetData.getString("percent_text", "0%") ?: "0%"
                setTextViewText(R.id.small_percent_text, percentText)

                // 프로그레스 바
                val progress = widgetData.getInt("progress", 0)
                setProgressBar(R.id.small_progress_bar, 100, progress, false)
                ...
                setOnClickPendingIntent(R.id.small_widget_container, pendingIntent)
            }
