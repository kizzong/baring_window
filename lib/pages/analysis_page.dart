...
class _AnalysisPageState extends State<AnalysisPage>
  ...
  int _completedCountForDay(
    ...
    for (final r in routines) {
      final isActive =
          r['type'] == 'daily' ||
          (r['type'] == 'weekly' &&
              (List<int>.from(r['days'] ?? [])).contains(weekday));
  ...
  List<_StreakData> _streakData() {
    ...
    }).toList()..sort((a, b) => b.current.compareTo(a.current));
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) {
        ...
        final activeDays = days
            .where((d) => s.type == 'daily' || s.activeDays.contains(d.weekday))
            .toList();
        ...
        final List<DateTime?> cells = [...List.filled(padFront, null), ...days];
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Text(
                    '완료',
                    style: TextStyle(fontSize: 12, color: sheetC.textSecondary),
                  ),
  ...
  Widget _buildMonthlyHeatmap(AppColors c) {
    ...
    final monthRate = monthTotal > 0
        ? (monthCompleted / monthTotal * 100).round()
        : 0;
      ...
      children: [
        ...
        AnimatedSwitcher(
          ...
          transitionBuilder: (child, animation) {
            final isNewChild =
                child.key ==
                ValueKey('${_selectedMonth.year}-${_selectedMonth.month}');
            final offset = Tween<Offset>(
              begin: Offset(
                isNewChild
                    ? _monthSlideDirection * 0.15
                    : -_monthSlideDirection * 0.15,
                0,
              ),
              end: Offset.zero,
  ...
  static const _helpTexts = {
    '오늘의 현황':
        '오늘 해야 할 할일과 루틴 중 몇 개를 완료했는지 보여줘요.\n\n'
        '큰 숫자는 완료한 개수, 아래는 전체 개수예요.\n\n'
        '예) 오늘 할일 3개 + 루틴 2개 = 총 5개\n'
        '그중 3개 완료 → "3 / 5" (60%)\n\n'
        '할일과 루틴은 각각 따로 표시돼요.',
    '연속 기록':
        '각 루틴의 연속 완료 기록을 보여줘요.\n\n'
    ...
    return Row(
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Icon(Icons.help_outline, size: 18, color: c.subtle),
        ),
      ],
    );
  }
...
class _BarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final maxVal = bars.fold<double>(
      0,
      (prev, b) => b.maxValue > prev ? b.maxValue : prev,
    );
    if (maxVal == 0) {
      return SizedBox(
        height: 140,
    ...
    return SizedBox(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: bars.map((b) {
          final totalHeight = maxVal > 0
              ? (b.maxValue / maxVal) * 100 * animValue
              : 0.0;
          final filledHeight = b.maxValue > 0
              ? (b.value / b.maxValue) * totalHeight
              : 0.0;
